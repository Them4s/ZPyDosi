...
def string_at(f, pos, length):
    ...
    #r = f.read(length).decode('utf8').rstrip('\00').rstrip()
    r = f.read(length)
    return r
...
def cnf2tka(path_cnf, path_tka, power_histo=False):
    out = open(path_tka,"w")
    c = read_cnf_file(path_cnf)
    if power_histo:
        out.write(str(int(c["Live time"]))+"\n")
        out.write("0\n")
    else:
        out.write(str(int(c["Live time"]))+"\n")
        out.write(str(int(c["Real time"]))+"\n")
    out.write("\n".join(map(lambda v: str(v), c["Channels data"][2:])))
    
    #list(c["Channels data"])
    #exit()
...
if __name__ == "__main__":
    ...
    c = read_cnf_file(filename, False)
