...
def icovar_jkk(l_l_v, l_w=None, do_cor=False, do_jackknife=False, fast_jackknife=True, blabla=False, nb_jkk=None, fast=True):
    ...
    if do_jackknife:
        ...
        jackknife_cov_var = np.fromiter((moy_without(i)-loc_cov)**2.0 for i in l_jkk).sum() * (len(l_l_v)/len(l_jkk)) * (len(l_l_v)-1.)/(len(l_l_v))
        #jackknife_cov_var = np.sum((dmoy_without(i))**2.0 for i in l_jkk) * (len(l_l_v)/len(l_jkk)) * (len(l_l_v)-1.)/(len(l_l_v))
        if do_cor:
            jackknife_cor_var = np.fromiter((moy_without_cor(i)-loc_cor)**2.0 for i in l_jkk).sum() * (len(l_l_v)/len(l_jkk)) * (len(l_l_v)-1.)/(len(l_l_v))
    t3 = time.time()
    #print t3-t2
